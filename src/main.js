...
class AdUnit extends Mads {
  render() {
	...
	postRender() {
		this.loadJS('https://code.createjs.com/createjs-2015.11.26.min.js').then(() => {
			this.loadJS(this.resolve('./js/300x250.js')).then(() => {
				lib.properties.manifest[0].src = this.resolve('./images/_2.jpg') + '?' + (+Date.now());
				lib.properties.manifest[1].src = this.resolve('./images/300x250_atlas_P_.png') + '?' + (+Date.now());
	      init();
	    });
		})
	}
  ...
  events() {
    var clickTag = 'http://www8.hp.com/id/en/printers/ink-advantage.html';
